...
public class Parser {
    ...
    public SyntaxNode parseCompUnit() { //CompUnit → {Decl} {FuncDef} MainFuncDef
        ...
        if (errorList.isEmpty()) { //有无错误直接看errorList里面有无内容就行了
            try (BufferedWriter stdout = new BufferedWriter(new FileWriter("D:\\BUAA_Compile_2024\\homework5\\src\\symbol.txt"))) {
            ...
            try (BufferedWriter stderr = new BufferedWriter(new FileWriter("D:\\BUAA_Compile_2024\\register-version\\src\\error.txt", true))) {
    ...
    public SyntaxNode parseFuncFParam(ArrayList<Integer> parasType) {
        ...
        if (!symbol.getIsArray()) { //加入形参类型
            ...
            if (TokenTypeMap.getInstance().getTokenType(type) == TokenType.INTTK) {
                parasType.add(1);
            } else if (TokenTypeMap.getInstance().getTokenType(type) == TokenType.CHARTK) {
                parasType.add(2);
            }
        }
        return node;
            ...
            if (peekToken().getTokenType() == TokenType.LPARENT) { //明确是调用function了
                ...
                if (peekToken().getTokenType() == TokenType.RPARENT) { //无参情况
                    ...
                    if (symbol.getIsArray()) {
                        if (TokenTypeMap.getInstance().getTokenType(symbol.getType()) == TokenType.INTTK) {
                            curParaType = 1;
                        } else if (TokenTypeMap.getInstance().getTokenType(symbol.getType()) == TokenType.CHARTK) {
                            curParaType = 2;
                        }
                    } else {
                        curParaType = 0;
